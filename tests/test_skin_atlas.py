...
def test_hd_skin_maps_faces_and_splits_limbs():
    ...
    plan = character_plan_from_skin_atlas(skin, name="sample", palette=PALETTE)

    assert set(_surface(plan, "head", SurfaceFace.FRONT).materials[0]) == {0}
    assert set(_surface(plan, "left_upper_arm", SurfaceFace.FRONT).materials[0]) == {0}
    assert set(_surface(plan, "left_forearm", SurfaceFace.FRONT).materials[0]) == {1}
    assert set(_surface(plan, "left_hand", SurfaceFace.FRONT).materials[0]) == {2}


def test_base_skin_quantizes_to_nearest_lab_palette_color():
    skin = Image.new("RGB", (64, 64), (245, 20, 20))

    plan = character_plan_from_skin_atlas(skin, name="sample", palette=PALETTE)

    assert _surface(plan, "torso", SurfaceFace.FRONT).materials[0][0] == 0


def test_optional_mask_maps_exact_grayscale_codes_to_relief():
    ...
    assert torso.relief[0][0] == -1
    assert torso.relief[0][5] == 0


def test_rejects_invalid_atlas_and_mask_inputs():
    ...
    with pytest.raises(ValueError, match="mask pixels"):
        character_plan_from_skin_atlas(
            skin, name="sample", palette=PALETTE, relief_mask=mask
        )
